...
def on_connect(client, userdata, flags, rc):
    ...
    if rc == 0:
        if not is_connected:  # Log message only when connecting for the first time
            log_display.insert(tk.END, "Client 2 connected successfully\n")
            is_connected = True
...
def send_message(event=None):
    ...
    # Create message to be sent (client2: message)
    message_to_send = f"client2:{message}"
...
recipient_menu = tk.OptionMenu(root, recipient_var, "client1", "client3")
