...
def on_connect(client, userdata, flags, rc):
    ...
    if rc == 0:
        if not is_connected:  # Log message only when connecting for the first time
            log_display.insert(tk.END, "Client 1 connected successfully\n")
            is_connected = True
...
def send_message(event=None):
    ...
    # Create message to be sent (client1: message)
    message_to_send = f"client1:{message}"
...
recipient_var.set("client2")  # Default value
recipient_menu = tk.OptionMenu(root, recipient_var, "client2", "client3")
